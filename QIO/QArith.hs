...
addBits qas qbs qc' =
  ...
  where
    addBits' [] [] qc = ifQ qc (unot qc')
    addBits' (qa : qas) (qb : qbs) qc =
      letU
        False
        ( \qc' ->
            carry qc qa qb qc'
              `mappend` addBits' qas qbs qc'
              `mappend` urev (carry qc qa qb qc')
        )
        `mappend` addBit qc qa qb
...
addBits' (qa : qas) (qb : qbs) (qc' : qcs') qc =
  ( carry qc qa qb qc'
      `mappend` addBits' qas qbs qcs' qc'
      `mappend` urev (carry qc qa qb qc')
  )
    `mappend` addBit qc qa qb
...
adder (QInt qas) (QInt qbs) = addBits qas qbs
        ...
        letU
          ...
          ( \qz ->
              letU
                ...
                ( \qc ->
                    adder qa qb qc
                      `mappend` urev (adder qn qb qc) -- b = a+b, c=False
                      `mappend` cond qc (\c -> if c then unot qz else mempty) -- b = a+b-N, c = a+b < N
                      `mappend` cond qz (\z -> if z then adder qn qb qc else mempty) -- z = c = a+b < N
                      `mappend` urev (adder qa qb qc) -- b = a+b mod N, c = False, z = a+b < N
                      `mappend` cond qc (\c -> if c then mempty else unot qz) -- if a+b < N then a=a,b=b,c=False
                      -- else a=a,b=a+b mod N - b,c=True
                      -- z = not c
                      `mappend` adder qa qb qc -- z = False
                      -- b = a+b mod N, c=False, z=False
                )
          )
    )
...
multMod n a (QInt x) y = multMod' n a x y 1
  where
    ...
    multMod' n a (x : xs) y p =
      cond
        x
        ( \x ->
            ( if x
                then letU ((p * a) `mod` n) (\qa -> adderMod n qa y) `mappend` multMod' n a xs y (p * 2)
...
modExpStep qc n a o p =
  letU
    0
    ( \z ->
        condMultMod qc n p' o z
          `mappend` ifQ qc (swapQInt o z)
          `mappend` urev (condMultMod qc n (inverseMod n p') o z)
    )
  where
    p'
      | a ^ (2 ^ p) == 0 = error "modExpStep: arguments too large"
