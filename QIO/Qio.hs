...
uMatrix q (m00, m01, m10, m11) =
  U
    ( \fv h ->
        ( if fromJust (h ? q)
            then
              m01
                <.> unEmbed (return (update h q False))
                <+> m11
                <.> unEmbed (return h)
            else
              m00
                <.> unEmbed (return h)
                <+> m10
                <.> unEmbed (return (update h q True))
        )
    )
...
evalWith (MkQbit b g) = do
  (StateQ f p) <- get
  put (StateQ (f + 1) (updateP p (Qbit f) b))
  evalWith (g (Qbit f))
evalWith (ApplyU u q) = do
  (StateQ f p) <- get
  put
    ( StateQ
        f
  ...
  where
    U uu = runU u
evalWith (Meas x g) = do
  (StateQ f p) <- get
  ( let Split pr ift iff = split p x
     in if pr < 0 || pr > 1
          then error "pr < 0 or >1"
          else do
            put (StateQ f ift)
