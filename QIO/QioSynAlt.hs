...
instance (Semigroup U) => Monoid U where
  mempty = Fx UReturn
...
newtype QIO a = Apply (Fix (QIOFunctor a))
...
instance Monad QIO where
  ...
  (Apply (Fx (MkQbit b g))) >>= f =
    Apply . Fx $
      MkQbit b (\q -> primQIO $ Apply (g q) >>= f)
  (Apply (Fx (ApplyU u q))) >>= f =
    Apply . Fx $
      ApplyU u $
        primQIO (Apply q >>= f)
  (Apply (Fx (Meas x g))) >>= f =
...
mkQbit b = Apply . Fx $ MkQbit b (primQIO . return)
...
measQbit x = Apply . Fx $ Meas x (primQIO . return)
...
count = cata count_algebra . primQIO
  where
    ...
    count_algebra (MkQbit b f) =
      let (mk, ap, ms) = f 0
       in (mk + 1, ap, ms)
    count_algebra (ApplyU _ (mk, ap, ms)) = (mk, ap + 1, ms)
    count_algebra (Meas q f) =
      let (mk, ap, ms) = f False
       in (mk, ap, ms + 1)
