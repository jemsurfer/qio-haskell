...
import qualified Control.Monad.RWS as Control.Monad
import Data.Complex
...
instance Monad QIO where
  return = pure
  (QReturn a) >>= f = f a
  (MkQbit b g) >>= f = MkQbit b (g Control.Monad.>=> f)
  (ApplyU u q) >>= f = ApplyU u (q >>= f)
  (Meas x g) >>= f = Meas x (g Control.Monad.>=> f)
...
spaces n =
  if n < 0
    then error "spaces: negative argument"
    else "    " ++ spaces (n - 1)
