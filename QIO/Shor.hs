...
randCoprime :: Int -> QIO Int
randCoprime n = do
  x <- randomQIO (0, length cps - 1)
  return (cps !! x)
  where
    cps = [x | x <- [0 .. n], gcd x n == 1]
...
randCo' n = do
  x <- randomQIO (2, n)
  if gcd x n == 1 then return x else randCo' n
